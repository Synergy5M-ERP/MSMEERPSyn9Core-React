...
export default function CreditNote() {
  ...
  const [saleinvoiceOptions, setsaleinvoiceOptions] = useState([]);
  ...
  const fetchDropdownData = async () => {
    setIsLoading(true);
    try {
      const [vendorRes, saleinvoicenoRes, itemRes] = await Promise.all([
        fetch(API_ENDPOINTS.Company),
        fetch(API_ENDPOINTS.Ledger),
        fetch(API_ENDPOINTS.BankDetails),
      ]);
      if (!vendorRes.ok || !saleinvoicenoRes.ok || !itemRes.ok) {
        throw new Error("Failed to fetch dropdown data");
      }
      const vendorData = await vendorRes.json();
      const saleinvoicenoData = await saleinvoicenoRes.json();
      const itemData = await itemRes.json();

      setVendorOptions(vendorData);
      setsaleinvoiceOptions(saleinvoicenoData);
  ...
  const handleSave = async () => {
    ...
    if (!saleinvoiceno) {
      toast.warn("Please select a sale order number");
      return;
    }
    if (!creditNoteDate) {
      toast.warn("Please select sale nullify date");
      return;
    }
    ...
    if (
      ...
      toast.warn("Please enter valid details for the primary sale item");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to save sale nullify");
      }

      await response.json();
      toast.success("sale nullify saved successfully!");
      setTimeout(() => {
        handleCancel();
      }, 1500);
    } catch (error) {
      console.error("Error saving sale nullify:", error);
      toast.error("Failed to save sale nullify. Please try again.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div style={{ maxHeight: 400, overflowY: "auto" ,fontFamily: "'Inter', sans-serif", padding: 20 }} className="custom-scrollbar">
      ...
      {/* Vendor & sale Order Selection */}
       ...
       <div className="row mb-3">
          ...
          <div className="col-3">
            <label style={{ display: "block", marginBottom: 6 }}>
              sale Order No <span style={{ color: "#ef4444" }}>*</span>
            </label>
            <select
              value={saleinvoiceno}
            ...
            >
              <option value="">Sale invoice no</option>
              {saleinvoiceOptions.map((o) => (
          ...
          <div className="col-3">
            <label style={{ display: "block", marginBottom: 6 }}>
              sale Nullify Date <span style={{ color: "#ef4444" }}>*</span>
            </label>
            <input
              type="date"
 ...
 <div className="col">
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 10,
            border: "none",
            background: editingIdx !== null ? "#f59e0b" : "green",
            color: "white",
            fontWeight: 600,
            fontSize: 16,
            cursor: "pointer",
      ...
      {creditNoteItems.length > 0 ? (
        ...
        >
          <p style={{ fontStyle: "italic", color: "#64748b", fontSize: 16 }}>
            No credit note items added yet. Add items to create the sale Nullify.
          </p>
        </section>
      )}
