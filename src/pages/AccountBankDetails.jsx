...
const [vendorId, setVendorId] = useState('');
    ...
    const fetchBanks = async (status = "active") => {
        ...
        try {
            let url = `${API_BASE_URL}/AccountBankDetails`;
 ...
 const selectedVendor = vendorNames.find(v => v.id === parseInt(vendorName));
  ...
  const handleEditSaved = (bank) => {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/AccountBankDetails/${editingId}`, {
 ...
 const handleSoftDelete = async (id) => {
    showAlert(
        ...
        async () => {
            setLoading(true);
            try {
                const response = await fetch(`${API_BASE_URL}/AccountBankDetails/${id}`, {
...
const handleActivate = async (id) => {
    ...
    try {
        ...
        if (result.success) {
            ...
            await fetchBanks(view);
        } else throw new Error(result.message || 'Activate failed');
    } catch (error) {
  ...
  const response = await fetch(`${API_BASE_URL}/AccountBankDetails`, {   // ✅ remove the extra /AccountBankDetails
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify(payload),
});
