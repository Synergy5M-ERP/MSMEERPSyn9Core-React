...
//         const mapped = sortedItems.map((r) => {
//           const totalAmount = Number(r.TotalAmount || r.totalAmount || r.TotalTaxAmount || 0);
//           const previouslyPaidAmount = Number(r.PaidAmount || r.paidAmount || 0); // ✅ Previously paid from DB
//           const balanceAmount = totalAmount - previouslyPaidAmount;
...
//             igst: Number(r.IGST || r.igst || 0),
//             totalAmount,
//             previouslyPaidAmount,  // ✅ Store previously paid amount separately
//             paidAmount: 0,         // ✅ New payment amount (starts at 0)
//             tillPaid: previouslyPaidAmount, // ✅ Till Paid = Previously paid
//             balanceAmount,
//             isChecked: balanceAmount === 0
//           };
//         });
...
//         // ✅ Calculate ledger balance based on NEW payments only
...
//   // ✅ FIXED: handlePaidChange with proper balance calculation
//   const handlePaidChange = (accountGRNId, value) => {
//     const numeric = Number(value);
//     const newPayment = Number.isNaN(numeric) || numeric < 0 ? 0 : numeric;

//     setRows(prevRows => {
//       const updated = prevRows.map(row => {
//         if (row.accountGRNId !== accountGRNId) return row;

//         // ✅ Calculate new Till Paid and Balance
//         const newTillPaid = row.previouslyPaidAmount + newPayment;
//         const newBalance = row.totalAmount - newTillPaid;

//         // ✅ Validate: Don't allow overpayment
//         if (newTillPaid > row.totalAmount) {
//           toast.warning(`⚠️ Payment cannot exceed balance amount of ₹${row.balanceAmount.toFixed(2)}`, {
//             toastId: `overpayment-${accountGRNId}`
//           });
//           return row;
//         }
...
//         return {
//           ...row,
//           paidAmount: newPayment,        // ✅ New payment amount (editable field)
//           tillPaid: newTillPaid,          // ✅ Total paid so far (previously + new)
//           balanceAmount: newBalance,      // ✅ Remaining balance
//           isChecked: newBalance === 0
//         };
//       });

//       // ✅ Update ledger balance based on NEW payments only
...
//         payments: selectedRows.map(row => ({
//           accountGRNId: row.accountGRNId,
//           paidAmount: row.tillPaid,  // ✅ Send total paid (previously + new)
//           totalAmount: row.totalAmount,
//           balanceAmount: row.balanceAmount,
//           cgst: row.cgst,
//           sgst: row.sgst,
...
//                               />
//                             </td>
//                             <td className="fw-semibold text-end align-middle text-info">
//                               ₹{row.tillPaid.toFixed(2)}
//                             </td>
//                             <td className="text-end align-middle">
//                               <span className={`badge fs-6 fw-semibold px-3 py-2 shadow-sm ${
//                                 row.balanceAmount === 0
