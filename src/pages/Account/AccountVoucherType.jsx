  ...
  function AccountVoucherType({ view }) {
    ...
    const [voucherategories, setVoucherCategories] = useState([]);
  ...
  if (formType === "vouchertype") {
    ...
    fetchVoucherCategories(); // 👈 ADD THIS
  } else {
  ...
  try {
    const res = await axios.get(API_ENDPOINTS.AccountVoucherCategory);
    const data = res.data.data || res.data || [];

    setVoucherCategories(
      data.map((v) => ({
        voucherCategoryId: v.voucherCategoryId,
        voucherCategory: v.voucherCategory,
      }))
    );
  } catch (err) {
    toast.error("Failed to load voucher categories");
  }
};
    ...
    const fetchVoucherTypes = async (status = "active") => {
      ...
      try {
        let url = API_ENDPOINTS.AccountVoucherType;
...
console.log("RAW API DATA:", raw);
        ...
        const mappedVoucherTypes = raw.map((l) => ({
          ...
          VoucherCategoryId: l.voucherCategoryId ?? null, 
    ...
    const fetchSubVoucherTypes = async (status = "active") => {
      setFetchLoading(true);
      try {
        let url = API_ENDPOINTS.AccountSubVoucherType;
        if (status === "active") url += "?isActive=true";
        else if (status === "inactive") url += "?isActive=false";
    ...
    const handleSave = async () => {
      if (formType === "vouchertype" && !voucherCategoryId) {
        toast.error("Please select Voucher Category");
        return;
      }
      ...
      setLoading(true);
      try {
        const baseUrl = formType === "vouchertype" ? API_ENDPOINTS.AccountVoucherType : API_ENDPOINTS.AccountSubVoucherType;
        ...
        if (formType === "vouchertype") {
          payload = {
            VoucherCategoryId: Number(voucherCategoryId),
            VoucherType: vouchertype.trim(),
            VoucherNarration: description.trim(),
            IsActive: true
          };
        } 
        if (formType === "subvouchertype") {
          payload = {
    ...
    const handleEdit = (item) => {
      if (formType === "vouchertype") {
       ...
       // setVoucherCategoryId(Number(item.VoucherCategoryId))     
       setVoucherCategoryId(item.VoucherCategoryId ? Number(item.VoucherCategoryId) : "")   
      ...
      try {
        ...
        if (formType === "vouchertype") {
          payload = {
            ...
            VoucherCategoryId: record.VoucherCategoryId,
    ...
    const LoadingSpinner = () => (
      <div
        ...
        style={{
          ...
          top: 0,left: 0, backgroundColor: "rgba(255,255,255,0.8)",zIndex: 2000,                   
        }}>
                ...
                {formType === "vouchertype" && (
                  <div className="mb-3">
                    ...
                    <select
                      ...
                      onChange={(e) => setVoucherCategoryId(Number(e.target.value))}
                      className="form-control"
                      disabled={loading}
                    >
                      <option value="">-- Select Voucher Category --</option>
                      {voucherategories.map((vc) => (
                        <option key={vc.voucherCategoryId} value={vc.voucherCategoryId}>
                          {vc.voucherCategory}
                        </option>
                      ))}
                    </select>
