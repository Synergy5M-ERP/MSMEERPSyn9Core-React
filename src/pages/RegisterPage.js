...
function Register() {
  ...
  // --- Dependent dropdowns ---
  useEffect(() => {
    if (!formData.source) return setContinents([]);
    axios
  ...
  useEffect(() => {
    if (!formData.continent) return setCountries([]);
    axios
      .get(`${apiBase}/GetCountry`, { params: { source: formData.source, continent: formData.continent } })
  ...
  useEffect(() => {
    ...
    axios
      .get(`${apiBase}/GetState`, { params: { source: formData.source, continent: formData.continent, country: formData.country } })
  ...
  useEffect(() => {
    ...
    axios
      .get(`${apiBase}/GetCity`, { params: { source: formData.source, continent: formData.continent, country: formData.country, state: formData.state } })
      .then(res => setCities(res.data || []))
      .catch(err => console.error("City fetch error:", err));
  }, [formData.state, formData.country, formData.continent, formData.source]);
        ...
        <div className="row g-3">
          ...
          <div className="col-md-4">
            <select name="authority" value={formData.authority} onChange={handleChange} className="form-control">
              ...
              {authorities.map(a => <option key={a.Authority_code} value={a.Authority_code}>{a.AuthorityName}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="row g-3">
          <div className="col-md-4">
            <select name="designation" value={formData.designation} onChange={handleChange} className="form-control">
              ...
              {designations.map(d => <option key={d.Designation_code} value={d.Designation_code}>{d.DesignationName}</option>)}
            </select>
          </div>
        </div>
