...
const fetchNextDebitNoteNo = async () => {
  try {
    const res = await axios.get(API_ENDPOINTS.GetNextDebitNoteNo);
    if (res.data?.success) {
      setDebitNoteNo(res.data.nextDebitNoteNo);
    }
  } catch (err) {
    toast.error("Failed to fetch Debit Note number");
  }
};
...
const fetchItemDetails = async (itemId) => {
  ...
  debitNoteEntries: items.map(it => ({
    ...
    setIsSaving(true);
    try {
      const response = await fetch(API_ENDPOINTS.SaveDebitNote, {
        method: "POST",
  ...
  return (
    ...
    >
      ...
      <div className="custom-scrollbar" style={{ maxHeight: 400, overflowY: "auto" }}>
        ...
        <div style={{
          padding: 28, borderRadius: 16, marginBottom: 32, border: "2px solid #e2e8f0"                 
        }}>
          <div className="row">
            <div className="col-3">
              <label
                style={{
                  textAlign: "left", display: "block", color: "#0066cc", fontSize: "14px", fontWeight: "600", 
                  marginBottom: "8px",
                }}>
            ...
            <div className="col-3">
              ...
              <input
                ...
                placeholder="CN-001"
                style={{
                  width: "100%", padding: "12px 16px", borderRadius: 10,border: "2px solid #e2e8f0", fontSize: 15, transition: "all 0.2s"       
                }} />
            </div>
